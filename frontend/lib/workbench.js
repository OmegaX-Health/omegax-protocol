...
function shortenGovernanceAddress(value) {
    if (value.length < 12)
        return value;
    return `${value.slice(0, 4)}...${value.slice(-4)}`;
}
function countBe(count) {
    return count === 1 ? "is" : "are";
}
function countRemain(count) {
...
const GOVERNANCE_QUEUE_STATE_ORDER = ["Voting", "Executing", "Completed"];
function describeGovernanceQueueStates(queueStateCounts) {
    return joinWithConjunction(Object.entries(queueStateCounts)
        .sort(([leftStatus], [rightStatus]) => {
        const leftIndex = GOVERNANCE_QUEUE_STATE_ORDER.indexOf(leftStatus);
        const rightIndex = GOVERNANCE_QUEUE_STATE_ORDER.indexOf(rightStatus);
...
function templateLabelForProposal(descriptionLink) {
    if (!descriptionLink)
...
function authorityLabelForProposal(proposal) {
    ...
    if (proposal.ownerRecordAddress)
        return `Record ${shortenGovernanceAddress(proposal.ownerRecordAddress)}`;
    return "Unavailable";
}
function stageLabelForProposal(proposal) {
...
export function sectionFromPathname(pathname) {
    ...
    if (pathname.startsWith("/governance"))
        return "governance";
    if (pathname.startsWith("/oracles"))
        return "oracles";
    if (pathname.startsWith("/plans"))
        return "plans";
    return "overview";
}
export function sectionChrome(section) {
    switch (section) {
        case "plans":
            return {
                title: "Plans",
...
export function derivePersonaFromRole(role) {
    switch (role) {
...
export function defaultTabForPersona(section, persona) {
    if (section === "plans") {
        if (persona === "capital")
            return "treasury";
...
export function linkedContextForPool(poolAddress) {
    const allocations = DEVNET_PROTOCOL_FIXTURE_STATE.allocationPositions.filter((row) => row.liquidityPool === (poolAddress ?? ""));
...
export function planAddressForSeries(seriesAddress) {
    return (DEVNET_PROTOCOL_FIXTURE_STATE.policySeries.find((series) => series.address === (seriesAddress ?? ""))?.healthPlan
        ?? null);
}
...
export function computeWorkbenchMetrics() {
    ...
    const reservedObligations = DEVNET_PROTOCOL_FIXTURE_STATE.obligations.filter((obligation) => obligation.status === OBLIGATION_STATUS_RESERVED || obligation.status === OBLIGATION_STATUS_CLAIMABLE_PAYABLE).length;
    const queueOnlyPools = DEVNET_PROTOCOL_FIXTURE_STATE.liquidityPools.filter((pool) => pool.redemptionPolicy === REDEMPTION_POLICY_QUEUE_ONLY).length;
    const approvedClaims = DEVNET_PROTOCOL_FIXTURE_STATE.claimCases.filter((claim) => claim.intakeStatus === CLAIM_INTAKE_APPROVED).length;
    return {
        activeClaims,
        approvedClaims,
        pendingRedemptions,
...
function compareAmountsDesc(left, right) {
...
function buildAuditTimestamp(seed, index) {
    let hash = 0;
...
function buildOverviewAuditTrail(persona = "observer", queue = []) {
    const metrics = computeWorkbenchMetrics();
    const leadProposal = queue[0] ?? null;
    const largestPool = [...DEVNET_PROTOCOL_FIXTURE_STATE.liquidityPools].sort((left, right) => compareAmountsDesc(left.totalValueLocked, right.totalValueLocked))[0] ?? null;
    const activePlan = DEVNET_PROTOCOL_FIXTURE_STATE.healthPlans.find((plan) => DEVNET_PROTOCOL_FIXTURE_STATE.claimCases.some((claim) => claim.healthPlan === plan.address && isActiveClaimStatus(claim.intakeStatus))) ?? DEVNET_PROTOCOL_FIXTURE_STATE.healthPlans[0] ?? null;
    if (persona === "capital") {
        const scopedAllocations = largestPool
            ...
            : [];
        const leadAllocation = [...scopedAllocations].sort((left, right) => compareAmountsDesc(left.allocatedAmount, right.allocatedAmount))[0] ?? null;
...
function buildCapitalAuditTrail(poolAddress, classAddress) {
    const pool = DEVNET_PROTOCOL_FIXTURE_STATE.liquidityPools.find((candidate) => candidate.address === (poolAddress ?? ""))
        ...
        ?? null;
    if (!pool)
        return buildOverviewAuditTrail("capital");
    ...
    const selectedClass = poolClasses.find((capitalClass) => capitalClass.address === (classAddress ?? ""))
        ?? poolClasses[0]
        ?? null;
    const scopedAllocations = DEVNET_PROTOCOL_FIXTURE_STATE.allocationPositions.filter((allocation) => selectedClass ? allocation.capitalClass === selectedClass.address : allocation.liquidityPool === pool.address);
    const leadAllocation = [...scopedAllocations].sort((left, right) => compareAmountsDesc(left.allocatedAmount, right.allocatedAmount))[0] ?? null;
    const leadSeries = DEVNET_PROTOCOL_FIXTURE_STATE.policySeries.find((series) => series.address === leadAllocation?.policySeries) ?? null;
    const leadPlan = DEVNET_PROTOCOL_FIXTURE_STATE.healthPlans.find((plan) => plan.address === leadAllocation?.healthPlan) ?? null;
    const scopedObligations = DEVNET_PROTOCOL_FIXTURE_STATE.obligations.filter((obligation) => obligation.liquidityPool === pool.address && (!selectedClass || obligation.capitalClass === selectedClass.address));
    const payableObligations = scopedObligations.filter((obligation) => obligation.status === OBLIGATION_STATUS_CLAIMABLE_PAYABLE);
    const payableAmount = payableObligations.reduce((sum, obligation) => sum + toBigIntAmount(obligation.payableAmount ?? obligation.outstandingAmount), 0n);
    ...
    const leadImpairedAllocation = [...impairedAllocations].sort((left, right) => compareAmountsDesc(left.impairedAmount, right.impairedAmount))[0] ?? null;
...
function buildPlansAuditTrail(planAddress, seriesAddress) {
    const plan = DEVNET_PROTOCOL_FIXTURE_STATE.healthPlans.find((candidate) => candidate.address === (planAddress ?? ""))
        ...
        ?? null;
    if (!plan)
        return buildOverviewAuditTrail("sponsor");
    ...
    const scopedClaims = DEVNET_PROTOCOL_FIXTURE_STATE.claimCases.filter((claim) => claim.healthPlan === plan.address && (!selectedSeries || claim.policySeries === selectedSeries.address));
    const scopedObligations = DEVNET_PROTOCOL_FIXTURE_STATE.obligations.filter((obligation) => obligation.healthPlan === plan.address && (!selectedSeries || obligation.policySeries === selectedSeries.address));
    const scopedMembers = DEVNET_PROTOCOL_FIXTURE_STATE.memberPositions.filter((member) => member.healthPlan === plan.address && (!selectedSeries || member.policySeries === selectedSeries.address));
    const scopedFundingLines = DEVNET_PROTOCOL_FIXTURE_STATE.fundingLines.filter((line) => line.healthPlan === plan.address && (!selectedSeries || line.policySeries === selectedSeries.address));
    const leadFundingLine = [...scopedFundingLines].sort((left, right) => compareAmountsDesc(availableFundingLineBalance(left), availableFundingLineBalance(right)))[0] ?? null;
    ...
    const outstandingObligations = scopedObligations.reduce((sum, obligation) => sum + toBigIntAmount(obligation.outstandingAmount ?? obligation.principalAmount), 0n);
...
function buildOraclesAuditTrail(poolAddress, seriesAddress) {
    const pool = DEVNET_PROTOCOL_FIXTURE_STATE.liquidityPools.find((candidate) => candidate.address === (poolAddress ?? ""))
        ?? DEVNET_PROTOCOL_FIXTURE_STATE.liquidityPools[0]
        ?? null;
    if (!pool)
        return buildOverviewAuditTrail("governance");
    ...
    const boundSeries = DEVNET_PROTOCOL_FIXTURE_STATE.policySeries.filter((series) => boundAllocations.some((allocation) => allocation.policySeries === series.address));
    const selectedSeries = boundSeries.find((series) => series.address === (seriesAddress ?? "")) ?? boundSeries[0] ?? null;
    const scopedClaims = DEVNET_PROTOCOL_FIXTURE_STATE.claimCases.filter((claim) => selectedSeries
        ? claim.policySeries === selectedSeries.address
        : boundSeries.some((series) => series.address === claim.policySeries));
    const scopedObligations = DEVNET_PROTOCOL_FIXTURE_STATE.obligations.filter((obligation) => selectedSeries
    ...
    const watchlistAmount = watchlistObligations.reduce((sum, obligation) => sum + toBigIntAmount(obligation.reservedAmount) + toBigIntAmount(obligation.payableAmount) + toBigIntAmount(obligation.impairedAmount), 0n);
...
function buildGovernanceAuditTrail(queue = [], proposal) {
    const selectedProposal = proposal ?? queue[0] ?? null;
    const queueStateCounts = queue.reduce((counts, item) => {
