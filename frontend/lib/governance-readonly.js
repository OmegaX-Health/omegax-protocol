...
import { InstructionExecutionStatus, ProposalState, ProposalTransaction, VoteThresholdType, YesNoVote, getAllTokenOwnerRecords, getGovernance, getGovernanceAccounts, getGovernanceProgramVersion, getNativeTreasuryAddress, getProposal, getProposalsByGovernance, getRealm, getTokenOwnerRecord, getTokenOwnerRecordsByOwner, getVoteRecordsByVoter, tryGetRealmConfig, } from "@solana/spl-governance";
...
const governanceBaseCache = createShortLivedPromiseCache({
    errorTtlMs: READONLY_GOVERNANCE_ERROR_TTL_MS,
    ttlMs: READONLY_GOVERNANCE_CACHE_TTL_MS,
});
const governanceProposalQueueCache = createShortLivedPromiseCache({
    errorTtlMs: READONLY_GOVERNANCE_ERROR_TTL_MS,
    ttlMs: READONLY_GOVERNANCE_CACHE_TTL_MS,
});
const governanceDashboardCache = createShortLivedPromiseCache({
    errorTtlMs: READONLY_GOVERNANCE_ERROR_TTL_MS,
    ttlMs: READONLY_GOVERNANCE_CACHE_TTL_MS,
});
const governanceProposalDetailCache = createShortLivedPromiseCache({
    errorTtlMs: READONLY_GOVERNANCE_ERROR_TTL_MS,
    ttlMs: READONLY_GOVERNANCE_CACHE_TTL_MS,
});
...
function toProposalStateLabel(state) {
    switch (state) {
...
function toExecutionStatusLabel(status) {
    switch (status) {
...
function classifyProposalGroup(state) {
    switch (state) {
        case ProposalState.Succeeded:
        case ProposalState.Executing:
        case ProposalState.ExecutingWithErrors:
...
function toGovernanceVoteChoice(voteRecord) {
    const yesNoVote = voteRecord.vote?.toYesNoVote();
    if (yesNoVote === YesNoVote.Yes)
        return "yes";
    if (yesNoVote === YesNoVote.No)
        return "no";
    return null;
}
function toProposalSummary(proposal, ownerRecordToWallet, walletVotes) {
    const communityThreshold = proposal.account.voteThreshold?.type === VoteThresholdType.YesVotePercentage
...
export function getGovernanceRuntimeConfig() {
    const cluster = process.env.NEXT_PUBLIC_REALMS_CLUSTER?.trim()
    ...
    return {
        ...
        governanceTokenMint: isConfigured(process.env.NEXT_PUBLIC_GOVERNANCE_TOKEN_MINT)
            ...
            : null,
        programId: process.env.NEXT_PUBLIC_GOVERNANCE_PROGRAM_ID?.trim()
            || DEFAULT_GOVERNANCE_PROGRAM_ID,
        programVersionOverride: overrideVersion != null && Number.isFinite(overrideVersion) && overrideVersion > 0
            ? overrideVersion
            : null,
...
function governanceRuntimeCacheKey(runtime) {
    return [
        runtime.cluster,
        runtime.programId,
...
async function resolveGovernanceProgramVersion(params) {
    const detectedVersion = await getGovernanceProgramVersion(params.connection, params.programId, params.cluster);
...
export function formatGovernanceAmount(raw, decimals) {
...
async function loadWalletGovernanceState(params) {
    const recordsByOwner = await getTokenOwnerRecordsByOwner(params.connection, params.programId, params.walletAddress).catch(() => []);
    const matchingRecord = recordsByOwner.find((record) => record.account.realm.equals(params.realmAddress)
        && record.account.governingTokenMint.equals(params.governanceTokenMint)) ?? null;
    const tokenAccountAddress = await getAssociatedTokenAddress(params.governanceTokenMint, params.walletAddress);
    const tokenAccount = await getAccount(params.connection, tokenAccountAddress).catch(() => null);
    const voteRecords = await getVoteRecordsByVoter(params.connection, params.programId, params.walletAddress).catch(() => []);
    const voteMap = new Map();
    for (const record of voteRecords) {
        const proposalAddress = record.account.proposal.toBase58();
        if (!params.proposalAddresses.has(proposalAddress))
            continue;
...
async function loadGovernanceBase(params) {
    return governanceBaseCache.getOrLoad(readonlyGovernanceCacheKey({
        ...
        const [realm, governance, nativeTreasuryAddress, nativeTreasuryLamports, mint, memberAccounts] = await Promise.all([
            ...
            nativeTreasuryAddressPromise.then((address) => params.connection.getBalance(address, "confirmed").then((lamports) => BigInt(lamports))),
        ...
        const ownerRecordToWallet = new Map();
...
export async function loadGovernanceProposalQueue(params) {
    return governanceProposalQueueCache.getOrLoad(readonlyGovernanceCacheKey({
        connection: params.connection,
        ...
        const governanceProgramId = new PublicKey(runtime.programId);
        const governanceAddress = new PublicKey(runtime.governanceAddress);
        const proposals = await getProposalsByGovernance(params.connection, governanceProgramId, governanceAddress);
        return proposals
            .map((proposal) => toProposalSummary(proposal, new Map(), new Map()))
            .sort((left, right) => right.address.localeCompare(left.address));
    });
}
export async function loadGovernanceDashboard(params) {
    return governanceDashboardCache.getOrLoad(readonlyGovernanceCacheKey({
        connection: params.connection,
        scope: "dashboard",
        ...
        return {
            ...
            rules: {
                ...
                communityVoteThresholdPct: base.governance.account.config.communityVoteThreshold.type === VoteThresholdType.YesVotePercentage
                ...
                minCommunityTokensToCreateGovernanceRaw: BigInt(base.realm.account.config.minCommunityTokensToCreateGovernance.toString(10)),
                minCommunityTokensToCreateProposalRaw: BigInt(base.governance.account.config.minCommunityTokensToCreateProposal.toString(10)),
                pluginEnabled: base.realm.account.config.useCommunityVoterWeightAddin
...
export async function loadGovernanceProposalDetail(params) {
    return governanceProposalDetailCache.getOrLoad(readonlyGovernanceCacheKey({
        connection: params.connection,
        ...
        const walletState = params.walletAddress
            ...
            : null;
        const proposalTransactions = (await getGovernanceAccounts(params.connection, base.governanceProgramId, ProposalTransaction)).filter((row) => row.account.proposal.equals(params.proposalAddress));
        const proposalOwnerRecord = await getTokenOwnerRecord(params.connection, proposal.account.tokenOwnerRecord).catch(() => null);
        const ownerRecordToWallet = new Map();
        if (proposalOwnerRecord) {
            ownerRecordToWallet.set(proposalOwnerRecord.pubkey.toBase58(), proposalOwnerRecord.account.governingTokenOwner.toBase58());
        }
        const currentWalletVoteMap = walletState?.voteMap ?? new Map();
