...
import {
  ...
  toBigIntAmount,
} from "./protocol/reserve-readiness";
...
export * from "./protocol/reserve-readiness";
...
export function buildAcceptGovernanceAuthorityTx(params: {
  ...
  return buildProtocolTransactionFromInstruction({
    ...
    args: {},
    accounts: [
      { pubkey: pendingAuthority, isSigner: true },
      { pubkey: deriveProtocolGovernancePda(), isWritable: true },
    ],
  });
}
...
export function buildOpenMemberPositionTx(params: {
  ...
  const inviteIdBytes = hexToFixedBytes(normalizeOptionalHex32(params.inviteIdHashHex), 32);
  const inviteAnchorRef = inviteIdBytes.some((byte) => byte !== 0)
    ? new PublicKey(inviteIdBytes)
    : null;
  const anchorRef = toPublicKey(params.anchorRefAddress ?? inviteAnchorRef ?? ZERO_PUBKEY_KEY);
  ...
  return buildProtocolTransactionFromInstruction({
    ...
    args: {
      ...
      invite_id_hash: Array.from(inviteIdBytes),
...
export function buildAuthorizeClaimRecipientTx(params: {
  ...
  delegateRecipientAddress: PublicKeyish;
  recentBlockhash: string;
}): Transaction {
  const authority = toPublicKey(params.authority);
  return buildProtocolTransactionFromInstruction({
    ...
    args: {
      delegate_recipient: toPublicKey(params.delegateRecipientAddress),
    },
    accounts: [
      { pubkey: authority, isSigner: true },
      { pubkey: deriveProtocolGovernancePda() },
...
function buildObligationFlowTx(params: {
  ...
  settlementAttestationAddress?: PublicKeyish | null;
  settlementOracleAddress?: PublicKeyish | null;
  args: Record<string, unknown>;
  ...
  const oracleFeeAttestation = params.oracleFeeAttestationAddress
    ...
    : params.oracleFeeAddress && params.claimCaseAddress
      ...
      : null;
  const settlementAttestation = params.settlementAttestationAddress
    ? toPublicKey(params.settlementAttestationAddress)
    : params.settlementOracleAddress && params.claimCaseAddress
      ? deriveClaimAttestationPda({
        claimCase: params.claimCaseAddress,
        oracle: params.settlementOracleAddress,
      })
  ...
  return buildProtocolTransactionFromInstruction({
    ...
    accounts: [
      ...
      ...(params.instructionName === "settle_obligation"
        ? [optionalProtocolAccount(settlementAttestation)]
        : []),
...
export function buildSettleClaimCaseTx(params: {
  ...
  const settlementAttestation = params.settlementAttestationAddress
    ? toPublicKey(params.settlementAttestationAddress)
    : params.settlementOracleAddress
      ? deriveClaimAttestationPda({
        claimCase: params.claimCaseAddress,
        oracle: params.settlementOracleAddress,
      })
...
export function buildUpdateLiquidityPoolControlsTx(params: {
  authority: PublicKeyish;
  poolAddress: PublicKeyish;
  recentBlockhash: string;
  pauseFlags: number;
  ...
  return buildProtocolTransactionFromInstruction({
    feePayer: authority,
    recentBlockhash: params.recentBlockhash,
    instructionName: "update_liquidity_pool_controls",
    args: {
      pause_flags: params.pauseFlags,
      active: params.active,
      reason_hash: Array.from(hexToFixedBytes(normalizeOptionalHex32(params.reasonHashHex), 32)),
