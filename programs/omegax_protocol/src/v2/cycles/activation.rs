...
pub fn activate_cycle_with_quote_sol(
    ...
    require_keys_eq!(
        ...
        OmegaXProtocolV2Error::MemberPayerMismatch
    );
    ...
    require!(premium_amount_raw > 0, OmegaXProtocolV2Error::InvalidAmount);
    require!(
        premium_amount_raw <= ctx.accounts.policy_series_payment_option.payment_amount,
        OmegaXProtocolV2Error::QuoteAmountExceedsConfiguredBase
    );
    require!(
    ...
    if commitment_enabled {
        let uses_legacy_threshold = threshold_bps > 0;
        ...
        require!(
            uses_legacy_threshold ^ uses_health_alpha_outcome,
            OmegaXProtocolV2Error::InvalidCycleQuote
        );
        require!(
    ...
    require!(now <= expires_at_ts, OmegaXProtocolV2Error::QuoteExpired);
    ...
    let instructions_sysvar = ctx
        .remaining_accounts
        .first()
        .ok_or(OmegaXProtocolV2Error::InvalidInstructionSysvar)?;
