...
pub fn withdraw_pool_treasury_spl(
    ...
    require!(
        amount <= available,
        OmegaXProtocolV2Error::InsufficientUnreservedTreasuryBalance
    );
...
pub fn pay_premium_onchain(
    ...
    require!(
        ctx.accounts.policy_position.pool == ctx.accounts.pool.key(),
        OmegaXProtocolV2Error::AccountPoolMismatch
    );
    require!(
        ctx.accounts.policy_position.member == ctx.accounts.member.key(),
...
pub fn pay_coverage_claim(ctx: Context<PayCoverageClaim>, payout_amount: u64) -> Result<()> {
    ...
    require_keys_eq!(
        ...
        ctx.accounts.claimant.key(),
        OmegaXProtocolV2Error::ClaimantMismatch
    );
    ...
    if ctx.accounts.pool_terms.payout_asset_mint == ZERO_PUBKEY {
        ...
        let asset_vault = &ctx.accounts.pool_asset_vault;
        require_keys_eq!(
            asset_vault.pool,
            pool_key,
            OmegaXProtocolV2Error::AccountPoolMismatch
        ...
        require!(
            ctx.accounts.recipient_token_account.owner == ctx.accounts.claimant.key(),
            OmegaXProtocolV2Error::RecipientMismatch
        );
        transfer_spl_reward(
            asset_vault,
    ...
    release_coverage_claim_liability(&mut ctx.accounts.pool_treasury_reserve, payout_amount)?;
    record_paid_coverage_claim(&mut ctx.accounts.pool_treasury_reserve, payout_amount)?;
    record_coverage_claim_payment(
        &mut ctx.accounts.coverage_claim,
        payout_amount,
        Clock::get()?.unix_timestamp,
    )?;
...
pub fn settle_coverage_claim(ctx: Context<SettleCoverageClaim>, payout_amount: u64) -> Result<()> {
    ...
    require!(
        ...
        OmegaXProtocolV2Error::CoverageClaimNotSubmitted
    );
    require_keys_eq!(
    ...
    release_coverage_claim_liability(&mut ctx.accounts.pool_treasury_reserve, payout_amount)?;
    record_paid_coverage_claim(&mut ctx.accounts.pool_treasury_reserve, payout_amount)?;
    record_coverage_claim_payment(&mut ctx.accounts.coverage_claim, payout_amount, now)
}
