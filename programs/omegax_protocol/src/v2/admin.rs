...
pub fn initialize_protocol_v2(
    ...
    require!(
        ...
        OmegaXProtocolV2Error::InvalidProtocolFee
    );
    require!(
        governance_realm != ZERO_PUBKEY,
        OmegaXProtocolV2Error::InvalidGovernanceRealm
    );
    require!(
        governance_config != ZERO_PUBKEY,
        OmegaXProtocolV2Error::InvalidGovernanceConfig
    );

    let config = &mut ctx.accounts.config_v2;
    ...
    config.bump = ctx.bumps.config_v2;
...
pub fn rotate_governance_authority(
    ...
    require!(
        ...
        OmegaXProtocolV2Error::InvalidGovernanceAuthority
    );
    ...
    if signer == config.governance_authority {
        ...
        require!(
            ...
            OmegaXProtocolV2Error::InvalidGovernanceRecoveryTarget
        );
        require_keys_eq!(
            new_authority,
...
pub fn update_oracle_profile_v2(
    ...
    require!(
        ...
        OmegaXProtocolV2Error::OracleProfileUnauthorized
    );
...
pub fn stake_oracle(ctx: Context<StakeOracle>, amount: u64) -> Result<()> {
    ...
    require!(
        ctx.accounts.stake_mint.key() == ctx.accounts.config_v2.default_stake_mint,
        OmegaXProtocolV2Error::InvalidStakeMint
    );
    ...
    if position.staker == ZERO_PUBKEY {
        ...
        require_keys_eq!(
            ...
            OmegaXProtocolV2Error::StakeVaultMismatch
        );
    }
    ...
    require!(
        position.staked_amount >= ctx.accounts.config_v2.min_oracle_stake,
...
pub fn request_unstake(
    ...
    require!(
        cooldown_seconds >= 0,
        OmegaXProtocolV2Error::InvalidCooldown
    );

    let position = &mut ctx.accounts.stake_position;
    require!(
        amount <= position.staked_amount,
        OmegaXProtocolV2Error::InsufficientStakeBalance
    );
...
pub fn finalize_unstake(ctx: Context<FinalizeUnstake>) -> Result<()> {
    ...
    require!(amount > 0, OmegaXProtocolV2Error::NoPendingUnstake);

    let now = Clock::get()?.unix_timestamp;
    require!(
        now >= ctx.accounts.stake_position.can_finalize_unstake_at,
        OmegaXProtocolV2Error::UnstakeCooldownNotMet
    );
