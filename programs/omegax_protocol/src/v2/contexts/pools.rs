...
pub struct SetPoolOraclePermissions<'info> {
    ...
    #[account(
        ...
        constraint = pool_oracle.pool == pool.key() @ OmegaXProtocolV2Error::AccountPoolMismatch,
        constraint = pool_oracle.oracle == oracle.key() @ OmegaXProtocolV2Error::OracleKeyMismatch,
    )]
...
pub struct FundPoolSpl<'info> {
    ...
    #[account(
        ...
        token::authority = pool_asset_vault,
    )]
