...
pub struct UpdatePolicySeriesV2<'info> {
    ...
    #[account(
        ...
        constraint = policy_series.pool == pool.key() @ OmegaXProtocolV2Error::AccountPoolMismatch,
    )]
    pub policy_series: Account<'info, PolicySeries>,
}
...
#[derive(Accounts)]
pub struct PayPremiumSplV2<'info> {
    #[account(mut)]
...
#[derive(Accounts)]
pub struct PayPremiumSolV2<'info> {
    #[account(mut)]
    pub payer: Signer<'info>,
    #[account(
        seeds = [SEED_CONFIG_V2],
...
#[derive(Accounts)]
#[instruction(series_ref_hash: [u8; 32], period_index: u64, amount: u64)]
pub struct PayPremiumOnchain<'info> {
    #[account(mut)]
    pub payer: Signer<'info>,
    #[account(
        seeds = [SEED_CONFIG_V2],
    ...
    #[account(
        ...
        payer = payer,
        space = PremiumLedger::space(),
        seeds = [SEED_PREMIUM_LEDGER, pool.key().as_ref(), &series_ref_hash, member.key().as_ref()],
        bump,
    )]
    pub premium_ledger: Box<Account<'info, PremiumLedger>>,
    #[account(
        seeds = [SEED_POOL_ASSET_VAULT, pool.key().as_ref(), pool_terms.payout_asset_mint.as_ref()],
    ...
    #[account(
        mut,
        constraint = payer_token_account.owner == payer.key() @ OmegaXProtocolV2Error::TokenAccountOwnerMismatch,
        constraint = payer_token_account.mint == pool_terms.payout_asset_mint @ OmegaXProtocolV2Error::PayoutMintMismatch,
    )]
    pub payer_token_account: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        ...
        constraint = pool_vault_token_account.mint == pool_terms.payout_asset_mint @ OmegaXProtocolV2Error::PayoutMintMismatch,
    )]
    pub pool_vault_token_account: Box<Account<'info, TokenAccount>>,
    pub token_program: Program<'info, Token>,
    pub system_program: Program<'info, System>,
...
pub struct ReviewCoverageClaim<'info> {
    ...
    #[account(
        mut,
        constraint = coverage_claim.pool == pool.key() @ OmegaXProtocolV2Error::AccountPoolMismatch,
    )]
