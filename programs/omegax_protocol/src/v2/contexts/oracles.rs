...
pub struct RegisterOracleV2<'info> {
    #[account(mut)]
...
#[derive(Accounts)]
pub struct UpdateOracleProfileV2<'info> {
...
pub struct FinalizeUnstake<'info> {
    ...
    #[account(
        ...
        constraint = stake_vault.owner == stake_position.key() @ OmegaXProtocolV2Error::StakeVaultMismatch,
        constraint = stake_vault.mint == stake_position.stake_mint @ OmegaXProtocolV2Error::InvalidStakeMint,
    )]
    pub stake_vault: Account<'info, TokenAccount>,
    #[account(
        mut,
        constraint = destination_token_account.mint == stake_position.stake_mint @ OmegaXProtocolV2Error::InvalidStakeMint,
    )]
...
pub struct SlashOracle<'info> {
    ...
    #[account(
        mut,
        constraint = slash_treasury_token_account.mint == stake_position.stake_mint @ OmegaXProtocolV2Error::InvalidStakeMint,
    )]
