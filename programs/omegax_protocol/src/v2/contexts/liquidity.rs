...
pub struct InitializePoolLiquiditySpl<'info> {
    ...
    #[account(
        mut,
        constraint = authority_payout_token_account.owner == authority.key() @ OmegaXProtocolV2Error::TokenAccountOwnerMismatch,
        constraint = authority_payout_token_account.mint == payout_mint.key() @ OmegaXProtocolV2Error::PayoutMintMismatch,
    )]
...
pub struct DepositPoolLiquiditySpl<'info> {
    ...
    #[account(
        ...
        constraint = pool_vault_token_account.mint == payout_mint.key() @ OmegaXProtocolV2Error::PayoutMintMismatch,
    )]
    pub pool_vault_token_account: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        constraint = depositor_payout_token_account.owner == depositor.key() @ OmegaXProtocolV2Error::TokenAccountOwnerMismatch,
        constraint = depositor_payout_token_account.mint == payout_mint.key() @ OmegaXProtocolV2Error::PayoutMintMismatch,
    )]
...
pub struct RedeemPoolLiquiditySol<'info> {
    ...
    #[account(
        mut,
        constraint = redeemer_share_token_account.owner == redeemer.key() @ OmegaXProtocolV2Error::TokenAccountOwnerMismatch,
        constraint = redeemer_share_token_account.mint == pool_share_mint.key() @ OmegaXProtocolV2Error::ShareMintMismatch,
    )]
...
pub struct RedeemPoolLiquiditySpl<'info> {
    ...
    #[account(
        mut,
        constraint = redeemer_payout_token_account.owner == redeemer.key() @ OmegaXProtocolV2Error::TokenAccountOwnerMismatch,
        constraint = redeemer_payout_token_account.mint == payout_mint.key() @ OmegaXProtocolV2Error::PayoutMintMismatch,
    )]
...
pub struct SchedulePoolLiquidityRedemption<'info> {
    ...
    #[account(
        mut,
        constraint = redemption_request.pool == pool.key() @ OmegaXProtocolV2Error::AccountPoolMismatch,
    )]
...
pub struct CancelPoolLiquidityRedemption<'info> {
    ...
    #[account(
        mut,
        constraint = redemption_request.pool == pool.key() @ OmegaXProtocolV2Error::AccountPoolMismatch,
        constraint = redemption_request.redeemer == redeemer.key() @ OmegaXProtocolV2Error::RedemptionRequestUnauthorized,
    )]
    pub redemption_request: Account<'info, PoolRedemptionRequest>,
    #[account(mut)]
    pub pool_share_mint: Account<'info, Mint>,
...
pub struct FailPoolLiquidityRedemption<'info> {
    ...
    #[account(
        mut,
        constraint = redeemer_share_token_account.owner == redemption_request.redeemer @ OmegaXProtocolV2Error::TokenAccountOwnerMismatch,
        constraint = redeemer_share_token_account.mint == pool_share_mint.key() @ OmegaXProtocolV2Error::ShareMintMismatch,
    )]
    pub redeemer_share_token_account: Account<'info, TokenAccount>,
    #[account(
...
pub struct FulfillPoolLiquidityRedemptionSpl<'info> {
    ...
    #[account(
        mut,
        constraint = redeemer_payout_token_account.owner == redemption_request.redeemer @ OmegaXProtocolV2Error::TokenAccountOwnerMismatch,
        constraint = redeemer_payout_token_account.mint == payout_mint.key() @ OmegaXProtocolV2Error::PayoutMintMismatch,
    )]
    pub redeemer_payout_token_account: Box<Account<'info, TokenAccount>>,
    #[account(
