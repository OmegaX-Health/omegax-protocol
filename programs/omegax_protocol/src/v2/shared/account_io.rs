...
pub(crate) fn serialize_program_account<T: AccountSerialize>(
    ...
    value
        ...
        .map_err(|_| OmegaXProtocolV2Error::InvalidProgramAccountData.into())
}

pub(crate) fn read_u16(data: &[u8], offset: usize) -> Result<u16> {
    let bytes = data
        .get(offset..offset + 2)
        .ok_or(OmegaXProtocolV2Error::InvalidQuoteSignatureInstruction)?;
...
pub(crate) fn verify_quote_signature(
    ...
    require_keys_eq!(
        ...
        OmegaXProtocolV2Error::InvalidInstructionSysvar
    );

    let current_index = load_current_index_checked(instructions_sysvar)?;
    require!(
        current_index > 0,
        OmegaXProtocolV2Error::MissingQuoteSignature
    );
    ...
    require!(
        data.len() >= 16,
        OmegaXProtocolV2Error::InvalidQuoteSignatureInstruction
    );
    require!(
        data[0] == 1,
        OmegaXProtocolV2Error::InvalidQuoteSignatureInstruction
    ...
    require!(
        ...
        OmegaXProtocolV2Error::QuoteMessageMismatch
    );
