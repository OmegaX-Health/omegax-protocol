...
pub(crate) fn total_reserved_treasury_balance(reserve: &PoolTreasuryReserve) -> Result<u64> {
    reserve
        ...
        .ok_or(OmegaXProtocolV2Error::MathOverflow.into())
}
...
pub(crate) fn upsert_cohort_settlement_root(
    ...
    if root.pool == ZERO_PUBKEY {
        ...
        require_keys_eq!(root.pool, pool, OmegaXProtocolV2Error::AccountPoolMismatch);
        require!(
            root.series_ref_hash == series_ref_hash,
        ...
        require!(
            root.outcome_threshold_score == outcome_threshold_score,
            OmegaXProtocolV2Error::OutcomeThresholdScoreMismatch
        );
    }
    Ok(())
}
...
pub(crate) fn compute_member_redistribution_share(
    ...
    require!(
        root.successful_health_alpha_score_sum > 0,
        OmegaXProtocolV2Error::HealthAlphaScoreRequired
    );
    require!(
    ...
    let share = numerator
        ...
        .ok_or(OmegaXProtocolV2Error::MathOverflow)?;
    u64::try_from(share).map_err(|_| OmegaXProtocolV2Error::MathOverflow.into())
}
...
pub(crate) fn available_treasury_balance(
    ...
    total_balance
        ...
        .ok_or(OmegaXProtocolV2Error::InsufficientUnreservedTreasuryBalance.into())
}
...
pub(crate) fn reward_claim_receipt_rent_lamports(payout_mint: Pubkey) -> Result<u64> {
    if payout_mint == ZERO_PUBKEY {
        return Ok(Rent::get()?.minimum_balance(ClaimRecordV2::space()));
    }
    Ok(0)
}
...
pub(crate) fn release_reward_liability_if_needed(
    ...
    if aggregate.reward_liability_reserved {
        reserve.reserved_reward_amount = reserve
            ...
            .ok_or(OmegaXProtocolV2Error::InsufficientReservedRewardBalance)?;
...
pub(crate) fn release_coverage_claim_liability(
    ...
    reserve.reserved_coverage_claim_amount = reserve
        .reserved_coverage_claim_amount
        .checked_sub(liability_amount)
        .ok_or(OmegaXProtocolV2Error::InsufficientReservedCoverageClaimBalance)?;
    touch_liability_ledger(reserve)
}
...
pub(crate) fn review_coverage_claim_case(
    ...
    require!(requested_amount > 0, OmegaXProtocolV2Error::InvalidAmount);
    require!(
        ...
        OmegaXProtocolV2Error::InvalidCoverageClaimFamily
    );
    require!(
        claim.status == COVERAGE_CLAIM_STATUS_SUBMITTED
            || claim.status == COVERAGE_CLAIM_STATUS_DENIED,
        OmegaXProtocolV2Error::InvalidCoverageClaimStateTransition
    );
...
pub(crate) fn approve_coverage_claim_case(
    ...
    require!(approved_amount > 0, OmegaXProtocolV2Error::InvalidAmount);
    require!(
        claim.status == COVERAGE_CLAIM_STATUS_UNDER_REVIEW,
        OmegaXProtocolV2Error::InvalidCoverageClaimStateTransition
    );
...
pub(crate) fn record_coverage_claim_payment(
    ...
    require!(
        ...
        OmegaXProtocolV2Error::CoverageClaimPayoutExceedsReservedAmount
    );
    ...
    claim.reserved_amount = claim
        ...
        .ok_or(OmegaXProtocolV2Error::CoverageClaimPayoutExceedsReservedAmount)?;
...
pub(crate) fn close_coverage_claim_case(
    ...
    require!(
        ...
        OmegaXProtocolV2Error::CoverageClaimAlreadyClosed
    );
    require!(
        claim.status == COVERAGE_CLAIM_STATUS_DENIED
            || claim.status == COVERAGE_CLAIM_STATUS_APPROVED
    ...
    if recovery_amount > 0 {
        ...
        require!(
            ...
            OmegaXProtocolV2Error::CoverageClaimRecoveryExceedsPaidAmount
        );
...
pub(crate) fn transfer_program_owned_lamports(
    ...
    require!(
        from_balance >= amount,
        OmegaXProtocolV2Error::InsufficientPoolBalance
    );
    ...
    require!(
        ...
        OmegaXProtocolV2Error::InsufficientPoolRentReserve
    );
...
pub(crate) fn transfer_spl_reward<'info>(
    ...
    require!(asset_vault.active, OmegaXProtocolV2Error::MissingAssetVault);
    require!(
