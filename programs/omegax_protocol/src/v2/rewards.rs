...
fn assert_valid_ai_role(ai_role: u8) -> Result<()> {
    require!(
        ...
        OmegaXProtocolV2Error::InvalidAiRole
    );
    Ok(())
}
...
fn ensure_outcome_claimable(
    ...
    match aggregate.review_status {
        ...
        OUTCOME_REVIEW_STATUS_PENDING_CHALLENGE => {
            err!(OmegaXProtocolV2Error::OutcomeChallengeWindowActive)
        }
        OUTCOME_REVIEW_STATUS_CHALLENGED => err!(OmegaXProtocolV2Error::OutcomeUnderDispute),
        OUTCOME_REVIEW_STATUS_OVERTURNED => err!(OmegaXProtocolV2Error::OutcomeDidNotPass),
        _ => err!(OmegaXProtocolV2Error::InvalidOutcomeReviewState),
    }
}
...
fn validate_attestation_automation(
    ...
    let policy = policy.ok_or(OmegaXProtocolV2Error::AutomationNotPermitted)?;
    require!(
        automation_mode <= policy.oracle_automation_mode,
        OmegaXProtocolV2Error::AutomationNotPermitted
    );
    require!(
        ai_role != AI_ROLE_NONE
...
fn validate_outcome_dispute_openable(aggregate: &CycleOutcomeAggregate, now: i64) -> Result<()> {
    require!(
        aggregate.finalized,
        OmegaXProtocolV2Error::OracleQuorumNotMet
    );
    require!(
        aggregate.review_status != OUTCOME_REVIEW_STATUS_CHALLENGED,
        OmegaXProtocolV2Error::OutcomeDisputeAlreadyOpen
    );
    require!(
        aggregate.challenge_window_ends_at == 0 || now <= aggregate.challenge_window_ends_at,
        OmegaXProtocolV2Error::OutcomeDisputeWindowClosed
    );
    Ok(())
}
...
pub fn submit_outcome_attestation_vote(
    ...
    require!(
        ...
        OmegaXProtocolV2Error::PoolNotActive
    );
    require!(
    ...
    require!(
        ...
        OmegaXProtocolV2Error::RuleDisabled
    );
    require!(
    ...
    require_keys_eq!(
        ...
        OmegaXProtocolV2Error::InvalidStakePosition
    );
    if ctx.accounts.config_v2.min_oracle_stake > 0
        || !ctx
            .accounts
            .stake_position
            .to_account_info()
            .data_is_empty()
    {
        ...
        require!(
            stake_position.staked_amount >= ctx.accounts.config_v2.min_oracle_stake,
            OmegaXProtocolV2Error::OracleInsufficientStake
        );
    }
    require_keys_eq!(
    ...
    if ctx.accounts.oracle_policy.require_verified_schema {
        require!(
            ...
            OmegaXProtocolV2Error::SchemaUnverified
        );
    }
    ...
    if aggregate.quorum_n == 0 {
        ...
        if !is_zero_hash(&aggregate.evidence_hash) && !is_zero_hash(&evidence_hash) {
            require!(
                aggregate.evidence_hash == evidence_hash,
                OmegaXProtocolV2Error::AttestationEvidenceMismatch
            );
            ...
            && !is_zero_hash(&external_attestation_ref_hash)
        {
            require!(
                aggregate.external_attestation_ref_hash == external_attestation_ref_hash,
                OmegaXProtocolV2Error::AttestationExternalReferenceMismatch
            );
    ...
    require!(
        !aggregate.finalized,
        OmegaXProtocolV2Error::OutcomeAlreadyFinalized
    );
    ...
    require!(
        ...
        OmegaXProtocolV2Error::OracleVoteWindowClosed
    );
...
pub fn submit_reward_claim(
    ...
    require!(payout_amount > 0, OmegaXProtocolV2Error::InvalidAmount);
    require!(
        ctx.accounts.membership.member == member,
        OmegaXProtocolV2Error::MembershipMemberMismatch
    );
    ...
    require!(
        ...
        OmegaXProtocolV2Error::OutcomeDidNotPass
    );
    require!(
        !ctx.accounts.aggregate.claimed,
        OmegaXProtocolV2Error::AlreadyClaimed
    );
    require!(
        ctx.accounts.aggregate.cycle_hash == cycle_hash,
        OmegaXProtocolV2Error::CycleHashMismatch
    );
    require!(
    ...
    require!(
        ctx.accounts.aggregate.series_ref_hash == ctx.accounts.claim_record_v2.series_ref_hash
            || ctx.accounts.claim_record_v2.pool == ZERO_PUBKEY,
    ...
    if let Some(member_cycle) = deserialize_optional_program_account::<MemberCycleState>(
        ...
        if member_cycle_uses_health_alpha_outcome(&member_cycle) {
            ...
            .ok_or(OmegaXProtocolV2Error::InvalidCohortSettlementRoot)?;
            ...
            require!(
                ...
                OmegaXProtocolV2Error::CohortSettlementNotFinalized
            );
            ...
            require!(
                ...
                OmegaXProtocolV2Error::CohortClaimCountExceeded
            );
            ...
            require!(
                ...
                OmegaXProtocolV2Error::RedistributionAmountMismatch
            );
    ...
    require!(
        ...
        OmegaXProtocolV2Error::PayoutAmountMismatch
    );
    ...
    authorize_claim_signer(
        ...
        ctx.accounts.claim_delegate.as_deref(),
    )?;
    ...
    if ctx.accounts.pool_terms.payout_asset_mint == ZERO_PUBKEY {
        ...
        let asset_vault = ctx
            ...
            .pool_asset_vault
            .as_ref()
            .ok_or(OmegaXProtocolV2Error::MissingAssetVault)?;
        let pool_vault_token_account = ctx
            .accounts
            .pool_vault_token_account
            .as_ref()
            .ok_or(OmegaXProtocolV2Error::MissingTokenAccount)?;
    ...
    let record = &mut ctx.accounts.claim_record_v2;
    ...
    record.bump = ctx.bumps.claim_record_v2;
...
pub fn resolve_cycle_outcome_dispute(
    ...
    require!(
        ctx.accounts.aggregate.review_status == OUTCOME_REVIEW_STATUS_CHALLENGED,
        OmegaXProtocolV2Error::OutcomeDisputeNotOpen
    );
...
mod tests {
    ...
    fn attestation_automation_rejects_missing_policy_and_provider_mismatch() {
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolV2Error::AutomationNotPermitted)
        );
    ...
    fn outcome_dispute_openable_respects_window_and_existing_state() {
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolV2Error::OutcomeDisputeWindowClosed)
        );
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolV2Error::OutcomeDisputeAlreadyOpen)
        );
    }
}
