...
pub struct WithdrawPoolTreasurySpl<'info> {
    ...
    #[account(
        ...
        constraint = pool_vault_token_account.mint == payment_mint.key() @ OmegaXProtocolError::PayoutMintMismatch,
    )]
...
pub struct WithdrawProtocolFeeSpl<'info> {
    ...
    #[account(
        mut,
        constraint = recipient_token_account.mint == payment_mint.key() @ OmegaXProtocolError::PayoutMintMismatch,
    )]
    pub recipient_token_account: Account<'info, TokenAccount>,
    pub token_program: Program<'info, Token>,
}
