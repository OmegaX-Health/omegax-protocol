...
pub struct FinalizeCycleOutcome<'info> {
    ...
    #[account(
        mut,
        constraint = aggregate.pool == pool.key() @ OmegaXProtocolError::AccountPoolMismatch,
    )]
    pub aggregate: Box<Account<'info, CycleOutcomeAggregate>>,
    pub system_program: Program<'info, System>,
}
...
pub struct SubmitRewardClaim<'info> {
    ...
    #[account(
        ...
        seeds = [SEED_CLAIM, pool.key().as_ref(), &aggregate.series_ref_hash, member.as_ref(), &cycle_hash, &rule_hash],
        bump,
    )]
