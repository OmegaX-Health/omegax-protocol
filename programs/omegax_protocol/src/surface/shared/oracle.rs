...
impl OracleProfileFields<'_> {
    pub(crate) fn validate(&self) -> Result<()> {
        require!(
            ...
            OmegaXProtocolError::InvalidOracleType
        );
        require!(
            self.display_name.len() <= MAX_ORACLE_DISPLAY_NAME_LEN,
            OmegaXProtocolError::OracleDisplayNameTooLong
        );
        require!(
            self.legal_name.len() <= MAX_ORACLE_LEGAL_NAME_LEN,
            OmegaXProtocolError::OracleLegalNameTooLong
        );
        require!(
            self.website_url.len() <= MAX_ORACLE_URL_LEN,
            OmegaXProtocolError::OracleUrlTooLong
        );
        require!(
        ...
        require!(
            ...
            OmegaXProtocolError::OracleLogoUriTooLong
        );
        require!(
            self.webhook_url.len() <= MAX_ORACLE_WEBHOOK_URL_LEN,
            OmegaXProtocolError::OracleWebhookUrlTooLong
        );
        require!(
            self.supported_schema_key_hashes.len() <= MAX_ORACLE_SUPPORTED_SCHEMAS,
            OmegaXProtocolError::OracleSupportedSchemaLimitExceeded
        );
        Ok(())
    }
}
...
pub(crate) fn require_oracle_claim_settle_permission(
    pool: Pubkey,
    oracle: Pubkey,
    pool_oracle: &PoolOracleApproval,
    pool_oracle_permissions: &PoolOraclePermissionSet,
...
pub(crate) fn oracle_role_required_permission(role: u8) -> Result<u32> {
    match role {
        ...
        _ => err!(OmegaXProtocolError::InvalidOracleRole),
    }
}
...
mod tests {
    ...
    fn rejects_oracles_without_data_attest_permission() {
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolError::OraclePermissionDenied)
        );
    }

    #[test]
    fn accepts_oracles_with_claim_settle_permission() {
        let pool = Pubkey::new_unique();
        let oracle = Pubkey::new_unique();
        let pool_oracle = sample_pool_oracle(pool, oracle, true);
        let permissions = sample_permission_set(pool, oracle, ORACLE_PERMISSION_CLAIM_SETTLE);

        assert!(
            require_oracle_claim_settle_permission(pool, oracle, &pool_oracle, &permissions)
                .is_ok()
        );
    }
