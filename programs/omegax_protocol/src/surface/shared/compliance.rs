...
pub(crate) fn assert_action_compliant(
    ...
    require_keys_eq!(policy.pool, pool, OmegaXProtocolError::AccountPoolMismatch);
    ...
    match policy.binding_mode {
        ...
        _ => err!(OmegaXProtocolError::InvalidComplianceBindingMode),
    }
}
    ...
    match policy.capital_rail_mode {
        ...
        RAIL_MODE_SPL_ONLY | RAIL_MODE_PERMISSIONED_SPL => {
            require!(
                ...
                OmegaXProtocolError::ComplianceRailRestriction
            );
            Ok(())
        }
        _ => err!(OmegaXProtocolError::InvalidRailMode),
    }
}
