...
pub(crate) fn pool_withdrawable_lamports(pool: &AccountInfo<'_>) -> Result<u64> {
    ...
    pool.lamports()
        ...
        .ok_or_else(|| error!(OmegaXProtocolError::InsufficientPoolRentReserve))
}
...
pub(crate) fn compute_deposit_shares_out(
    ...
    require!(reserves_before > 0, OmegaXProtocolError::ZeroReserves);
    ...
    let shares_out_u128 = (amount_in as u128)
        ...
        .ok_or(OmegaXProtocolError::MathOverflow)?;
    let shares_out =
        u64::try_from(shares_out_u128).map_err(|_| OmegaXProtocolError::MathOverflow)?;
    require!(shares_out > 0, OmegaXProtocolError::InsufficientSharesOut);
    Ok(shares_out)
}
...
pub(crate) fn compute_redeem_amount_out(
    ...
    require!(shares_supply > 0, OmegaXProtocolError::ZeroSharesSupply);
    require!(reserves_before > 0, OmegaXProtocolError::ZeroReserves);
    ...
    let amount_out_u128 = (shares_in as u128)
        ...
        .ok_or(OmegaXProtocolError::MathOverflow)?;
    let amount_out =
        u64::try_from(amount_out_u128).map_err(|_| OmegaXProtocolError::MathOverflow)?;
    require!(amount_out > 0, OmegaXProtocolError::SlippageExceeded);
    Ok(amount_out)
}
...
pub(crate) fn build_capital_class_accounting_snapshot(
    ...
    let utilization_bps = if total_balance_before == 0 {
        ...
        let utilization = (encumbered_capital as u128)
            ...
            .ok_or(OmegaXProtocolError::MathOverflow)?
            .min(10_000);
        u16::try_from(utilization).map_err(|_| OmegaXProtocolError::MathOverflow)?
    };
...
pub(crate) fn schedule_redemption_request(
    ...
    require!(
        ...
        OmegaXProtocolError::InvalidRedemptionRequestState
    );
    require!(
        now >= request.notice_matures_at,
        OmegaXProtocolError::RedemptionRequestNotMatured
    );
...
mod tests {
    ...
    fn redeem_amount_out_rejects_underwater_treasury() {
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolError::InsufficientUnreservedTreasuryBalance)
        );
    }
    ...
    fn redeem_amount_out_rejects_when_no_free_capital_remains() {
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolError::ZeroReserves)
        );
    }
    ...
    fn redemption_request_schedule_requires_pending_and_mature_notice() {
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolError::RedemptionRequestNotMatured)
        );
        ...
        assert_eq!(
            error_code(error),
            omega_error_code(OmegaXProtocolError::InvalidRedemptionRequestState)
        );
    }
