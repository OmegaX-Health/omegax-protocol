...
pub(crate) fn deposit_commitment(
    ...
    require_keys_eq!(
        ...
        ctx.accounts.campaign.payment_asset_mint,
        OmegaXProtocolError::AssetMintMismatch
    );
    require_keys_eq!(
    ...
    let amount = ctx.accounts.campaign.deposit_amount;
    ...
    if ctx.accounts.campaign.hard_cap_amount > 0 {
        require!(
            checked_add(accepted_amount, amount)? <= ctx.accounts.campaign.hard_cap_amount,
    ...
    position.payment_asset_mint = ctx.accounts.campaign.payment_asset_mint;
    position.coverage_asset_mint = ctx.accounts.campaign.coverage_asset_mint;
    position.amount = amount;
    position.coverage_amount = ctx.accounts.campaign.coverage_amount;
...
pub(crate) fn activate_direct_premium_commitment(
    ...
    require!(
        ctx.accounts.campaign.mode == COMMITMENT_MODE_DIRECT_PREMIUM,
    ...
    emit!(CommitmentActivatedEvent {
        ...
        mode: ctx.accounts.campaign.mode,
    });
...
pub(crate) fn activate_treasury_credit_commitment(
    ...
    require!(
        ctx.accounts.campaign.mode == COMMITMENT_MODE_TREASURY_CREDIT,
        OmegaXProtocolError::InvalidCommitmentCampaignMode
    );
    require_keys_neq!(
        ctx.accounts.campaign.payment_asset_mint,
        ctx.accounts.campaign.coverage_asset_mint,
        OmegaXProtocolError::TreasuryCreditAssetMismatch
    );
    require_pending_commitment_position(&ctx.accounts.position)?;
    require_matching_position(
...
fn require_commitment_mode(mode: u8) -> Result<()> {
    match mode {
        COMMITMENT_MODE_DIRECT_PREMIUM | COMMITMENT_MODE_TREASURY_CREDIT => Ok(()),
...
fn require_matching_position(
    ...
    require_keys_eq!(
        ...
        campaign.payment_asset_mint,
        OmegaXProtocolError::AssetMintMismatch
    );
    require_keys_eq!(
        position.coverage_asset_mint,
        campaign.coverage_asset_mint,
        OmegaXProtocolError::AssetMintMismatch
    );
    Ok(())
}
...
fn require_matching_coverage_line(
    ...
    require_keys_eq!(
        campaign.coverage_funding_line,
        funding_line_key,
        OmegaXProtocolError::FundingLineMismatch
    );
    require_keys_eq!(
...
pub struct DepositCommitment<'info> {
    ...
    pub campaign: Box<Account<'info, CommitmentCampaign>>,
    #[account(mut, seeds = [SEED_COMMITMENT_LEDGER, campaign.key().as_ref(), campaign.payment_asset_mint.as_ref()], bump = ledger.bump)]
    pub ledger: Box<Account<'info, CommitmentLedger>>,
    #[account(
        init,
        payer = depositor,
    ...
    #[account(mut, seeds = [SEED_DOMAIN_ASSET_VAULT, campaign.reserve_domain.as_ref(), campaign.payment_asset_mint.as_ref()], bump = domain_asset_vault.bump)]
...
pub struct ActivateDirectPremiumCommitment<'info> {
    ...
    pub position: Box<Account<'info, CommitmentPosition>>,
    #[account(mut, seeds = [SEED_DOMAIN_ASSET_LEDGER, health_plan.reserve_domain.as_ref(), campaign.coverage_asset_mint.as_ref()], bump = coverage_domain_asset_ledger.bump)]
    pub coverage_domain_asset_ledger: Box<Account<'info, DomainAssetLedger>>,
    ...
    pub coverage_funding_line: Box<Account<'info, FundingLine>>,
    #[account(mut, seeds = [SEED_FUNDING_LINE_LEDGER, coverage_funding_line.key().as_ref(), campaign.coverage_asset_mint.as_ref()], bump = coverage_funding_line_ledger.bump)]
    pub coverage_funding_line_ledger: Box<Account<'info, FundingLineLedger>>,
    #[account(mut, seeds = [SEED_PLAN_RESERVE_LEDGER, health_plan.key().as_ref(), campaign.coverage_asset_mint.as_ref()], bump = coverage_plan_reserve_ledger.bump)]
    pub coverage_plan_reserve_ledger: Box<Account<'info, PlanReserveLedger>>,
    #[account(mut)]
